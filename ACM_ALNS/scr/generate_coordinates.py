...
def generate_graph(position_points,seed,uav_distance=None):
    ...
    # 计算节点之间的距离并添加边
    for i in range(len(coordinates_air)):
        for j in range(i + 1, len(coordinates_air)):
            # 使用欧氏距离作为权重
            distance_air = coordinates_air_matrix[i,j]
            prob_air = connect_prob(connect_count_air[i], distance_air, max_distance_air)
            if random.random() < prob_air:
                G_air.add_edge(i,j, weight=distance_air)
                connect_count_air[i] += 1
                connect_count_air[j] += 1
                if connect_count_air[i] >= 2:
                    break
    # 创建地面无向图
...
# 建立塔杆节点连接概率函数
def connect_prob(connections, distance, max_distance):
    # 计算连接概率
    base_prob = 1.0
    prob = base_prob * (1-0.2*connections)
    distance_prob = distance / max_distance
    prob = np.mean(prob + distance_prob)
    return prob
